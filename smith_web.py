...
"""
import os, base64, threading, webbrowser, urllib.parse, uuid, json, subprocess, socket, hashlib, logging, secrets

from flask import Flask, request, jsonify, send_from_directory, session, redirect, render_template, Response, stream_with_context
import assistant
import re as _re
import db as _db
...
def _md_to_html(text):
    if _md:
        ...
        html = _re.sub(
            ...
            ) % ((m.group(2) or "code").upper(), m.group(3)),
    ...
    def repl(m):
        ...
        return ('<div class="code-block"><div class="code-header">'
                '<span class="code-lang">%s</span>'
                '<button class="code-copy-btn" onclick="copyCode(this)">Copy</button>'
                '</div><pre><code>%s</code></pre></div>') % (lang, code)
    return _re.sub(r'```(\w*)\n([\s\S]*?)```', repl, text)
try:
...
_vmem.init()
  # set up vector/FTS memory tables
...
# ── secret key: generate once, persist ───────────────────────────
_SECRET_FILE = os.path.join(BASE, ".secret_key")
def _get_secret_key():
...
def _check_pw(pw, stored):
    # support legacy SHA-256 hashes so existing users aren't locked out
...
def _cap(text):
    try:
        ...
        log.debug("Could not remember turn: %s", e)
...
def _web_answer(question, results):
    ...
    try:
        ...
        log.warning("Web answer error: %s", e)
        return None
...
def _deep_research(query):
    ...
    try:
        ...
        log.warning("Sub-query decomp error: %s", e)
...
def save_chat(cid, title, messages):
    try:
        _db.save_chat(cid, _current_user() or "default", title, messages)
    except Exception as e:
...
def load_chat(cid):
    try:
        ...
        log.debug("Could not load chat %s: %s", cid, e)
        return None


def list_chats():
    try:
        return _db.list_chats(_current_user() or "default")
    except Exception as e:
        log.warning("list_chats failed: %s", e)
        return []
...
def login():
    if request.method == "POST":
        ...
        stored = _db.get_user(un)
        if un and stored:
            pw_hash = stored["pw_hash"]
            if _check_pw(pw, pw_hash):
    ...
    if request.method == "POST":
        ...
        if not un or not pw:
            ...
            if _db.get_user(un):
                ...
                _db.create_user(un, nick, _hash_pw(pw))
                session["auth"] = True
                session["username"] = un
                session["nickname"] = nick
                session.permanent = True
...
"""
            ...
            if needs_search:
                try:
                    ...
                    assistant.answer(msg)
            elif any(t in msg.lower() for t in [
                    "create an image", "generate an image", "make an image",
                    "draw an image", "generate image of", "create image of",
                    "make image of", "draw a picture", "create a picture",
                    "generate a picture", "make a picture", "create a photo",
                    "generate a photo", "paint a", "draw a", "render an image",
                    "image of ", "picture of ", "illustration of ",
            ]):
                # Direct Pollinations image generation
                try:
                    img_path = assistant.create_image(msg)
                    if img_path and os.path.exists(img_path):
                        ...
                        _capture.append("[IMAGE]/uploads/" + fname)
                    else:
                        _capture.append("Sorry, image generation failed. Try again with a different prompt.")
                except Exception as _ie:
                    log.error("Image generation error: %s", _ie)
                    _capture.append("Image generation error: " + str(_ie))
            else:
        ...
        if reply.startswith("[IMAGE]"):
            ...
            try:
                ...
                log.warning("Image copy failed: %s", e)
...
# /stream → routes/stream_routes.py (Blueprint)
...
def memory_route():
    ...
    if request.method == "GET":
        return jsonify({"memories": _db.get_memories(_current_user() or "default")})
    body = request.json or {}
    action = body.get("action", "")
    if action == "save":
        fact = body.get("fact", "").strip()
        if fact:
            _db.add_memory(_current_user() or "default", fact)
        return jsonify({"ok": True, "memories": _db.get_memories(_current_user() or "default")})
    if action == "clear":
        _db.clear_memories(_current_user() or "default")
...
def tools_custom_save():
    """Save a custom tool Python script to tools/ directory."""
    if not _current_user():
        return jsonify({"error": "login"}), 401
    body = request.json or {}
    ...
    # Safety: only allow simple filenames, force .py extension
    safe_name = _re.sub(r"[^a-zA-Z0-9_]", "_", filename.replace(".py", "")) + ".py"
    ...
    code_low = code.lower()
    blocked = [d for d in danger if d in code_low]
    if blocked:
        return jsonify({"error": f"Custom tools cannot use: {', '.join(blocked)}"}), 400
    try:
        with open(tool_path, "w", encoding="utf-8") as f:
            f.write(code)
        _tools.reload()
        return jsonify({"ok": True, "filename": safe_name,
                        "message": f"Tool saved as '{safe_name}' and loaded."})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/reminders/due", methods=["GET"])
def reminders_due():
    """Poll for any fired reminders — called by frontend every 60s."""
    if not _current_user():
        return jsonify({"reminders": []})
    try:
        from tools.reminders import get_due_reminders
        return jsonify({"reminders": get_due_reminders(_current_user())})
    except Exception as e:
        log.warning("reminders_due error: %s", e)
        return jsonify({"reminders": []})

@app.route("/settings/personality", methods=["GET", "POST"])
def personality_route():
    if not _current_user():
        return jsonify({"error": "login"}), 401
    uname = _current_user()
    if request.method == "GET":
        return jsonify(_db.get_settings(uname))
    body = request.json or {}
    _db.save_settings(
        uname,
...
@app.route("/memory/vector_clear", methods=["POST"])
def vector_clear_route():
    if not _current_user():
        return jsonify({"error": "login"}), 401
    _vmem.clear_user_memory(_current_user())
    return jsonify({"ok": True})
...
def delete_chat_route(cid):
    if not _current_user():
        return jsonify({"error": "login"}), 401
    _db.delete_chat(cid, _current_user())
    return jsonify({"ok": True})


@app.route("/chats/<cid>/rename", methods=["POST"])
def rename_chat_route(cid):
    if not _current_user():
        return jsonify({"error": "login"}), 401
    title = (request.json or {}).get("title", "").strip()
    if title:
        _db.rename_chat(cid, _current_user(), title)
    return jsonify({"ok": True})
...
WORKSPACE_DIR = os.path.normpath(os.getenv("WORKSPACE_DIR", os.path.join(BASE, "workspace")))
...
def _safe_path(rel: str):
    """Resolve a relative path inside WORKSPACE_DIR; return None on traversal."""
    full = os.path.normpath(os.path.join(WORKSPACE_DIR, rel.lstrip("/\\")))
...
def files_list():
    ...
    base    = _safe_path(dir_rel) if dir_rel else WORKSPACE_DIR
...
def files_read():
    ...
    if ext not in _TEXT_EXTS:
        return jsonify({"error": f"Binary/unsupported file type: .{ext}"}), 415
